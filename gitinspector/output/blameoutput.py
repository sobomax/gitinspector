...
BLAME_INFO_TEXT = N_("Below are the number of lines of code from each author that have survived and are still "
...
class BlameOutput(Outputable):
	...
	def output_html(self):
		...
		blame_xml += "<thead><tr> <th>{0}</th> <th>{1}</th> <th>{2}</th> <th>{3}</th> <th>{4}</th> </tr></thead>".format(
		             _("Author"), _("Lines of Code"), _("Stability"), _("Age"), _("% in comments"))
		blame_xml += "<tbody>"
		chart_data = ""
	...
	def output_text(self):
		...
		terminal.printb(terminal.ljust(_("Author"), 21) + terminal.rjust(_("Lines of Code"), 10) + terminal.rjust(_("Stability"), 15) +
